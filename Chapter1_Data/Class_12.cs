...
namespace CSharp_ProgramingStudy.Chapter1_Data
{
    ...
    public class Class12
    {
        public void Run()
        {
            ...
            Console.WriteLine($"전위 증가 연산: {++number}");  // 출력: 6

            // 후위 증감 연산자: 값을 먼저 사용하고 그 후에 증가시킴
            Console.WriteLine($"후위 증가 연산: {number++}");  // 출력: 6 (이후 number는 7이 됨)
            Console.WriteLine($"number 값: {number}");         // 출력: 7

            // 전위 감소 연산자: 먼저 값을 감소시킨 후 사용
            Console.WriteLine($"전위 감소 연산: {--number}");  // 출력: 6

            // 후위 감소 연산자: 값을 먼저 사용하고 그 후에 감소시킴
            Console.WriteLine($"후위 감소 연산: {number--}");  // 출력: 6 (이후 number는 5가 됨)
            Console.WriteLine($"number 값: {number}");         // 출력: 5
        }
    }
}
