...
namespace CSharp_ProgramingStudy.Chapter2_CodeFlow
{
  public class Chapter2_Quiz
  {
      ...
      // - 멤버십 할인: 멤버십 회원(isMember)이면서, 현재 결제 금액이 13,000원 이상인 경우 최종 금액에서 2,000원를 할인합니다.
      static void Main(string[] args)
      {
          int basePrice = 15000;
          int age = 25;           // 테스트용 나이
          bool isWeekday = false; // 테스트용 요일 (true: 평일)
          bool isMember = true; // 테스트용 멤버십
          ...
          // 멤버십 할인: 멤버십 회원이면서 결제 금액이 13,000원 이상 (&& 연산자)
          if (isMember && finalPrice >= 13000)
          {
              finalPrice = finalPrice - 2000;
          }
